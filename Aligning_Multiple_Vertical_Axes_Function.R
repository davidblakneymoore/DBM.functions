...
#' 
#' This function compares 3 or more correlation coefficients based on a well established and published method (Levy, 1977). Importantly, it assumes the correlations are independent. When independence cannot be assumed, please consider using a different procedure. This function only uses [base] R functions, but it was heavily inspired by the [agricolae] package, particularly the [orderPvalue()] and [lastC()] functions (de Mendiburu, 2017).
#' 
#' `Aligning_Values_Across_Multiple_Vertical_Axes_Function` determines how to align values optimally across multiple vertical axes such that there isn't unnecessary  additional space on the top or the bottom of any graph and such that the specified values will align perfectly across all vertical axes.
...
#' 
#' @return the new axis limits for all the vertical axes being aligned.
#' 
