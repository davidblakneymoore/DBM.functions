#' Optimally Assigning Experimental Units to Treatment Groups for Completely Randomized Designs
#'
#' This function may be used to assign experimental units to treatment groups for completely randomized designs when you wish to ensure that treatment groups are as balanced as possible for a particular set of experimental units' variables.
#'
#' `Optimally_Assigning_Experimental_Units_to_Treatment_Groups_for_Completely_Randomized_Designs` may be used when setting out a completely-randomized-design study or experiment to optimize the assignments of experimental units to treatment groups. Although randomization is typically used to assign treatments to experimental units, sometimes, randomization may unintentionally create treatment groups that are not balanced. For example, perhaps after randomizing, certain treatment groups end up being comprised of experimental units that are bigger or heavier or hotter or shadier or wetter than the experimental units in the other treatment groups are. This undesirable effect may be avoided by using this function, which ensures that means and possibly other higher-order mathematical moments (such as variances, skewnesses, and kurtoses) for a particular set of experimental units' variables you've already measured are as similar as possible across treatment groups.
#'
#' Please note that randomization is still a very good technique. If you suspect that you did not measure a suite of experimental unit variables that may influence the response variables in your study or experiment, randomization may be a better method than this optimization method.
#'
...
#' @param Moment_Weighing_Method the method of weighing mathematical moments. Almost all of the built-in methods weigh moments sequentially, with lower-order moments being weighed more heavily than higher-order moments are weighed. This function calculates moments up to the order of the number of experimental units in each treatment group (as specified in the `Number_of_Experimental_Units_in_Each_Treatment_Group` argument). The default method weighs each moment twice as heavily as the subsequent moment, with lower-order moments being weighed the most heavily and higher-order moments being weighed the least heavily.
...
#' @param Number_of_Combinations_to_Report the number of grouping assignments to report. Grouping assignments are always reported such that the best (most optimal) assignments are reported first. By default, for most applications, the top 1 % of grouping assignments are reported.
#'
#' @return This function returns optimal treatment group assignments for completely randomized designs.
#'
#' @author David B. Moore (\email{davidblakneymoore@@gmail.com})
#'
#' @examples
#' set.seed(13)
#' Data_Frame <- data.frame(Tree_Identifier = paste("Tree", seq_len(13), sep = "_"), Diameter_at_Breast_Height = round(abs(rnorm(13, 30, 2.5)), 1), Height = round(abs(rnorm(13, 30, 2.5)), 1), Crown_Class = sample(c("Dominant", "Codominant", "Intermediate", "Overtopped"), 13, replace = TRUE))
#' (Optimal_Group_Assignments <- Optimally_Assigning_Experimental_Units_to_Treatment_Groups_for_Completely_Randomized_Designs(Diameter_at_Breast_Height, Height, Crown_Class, Identifiers = Tree_Identifier, Data_Frame = Data_Frame, Number_of_Combinations_to_Report = 3, Number_of_Treatment_Groups = 3, Number_of_Experimental_Units_in_Each_Treatment_Group = 4))
#'
#' @export
Optimally_Assigning_Experimental_Units_to_Treatment_Groups_for_Completely_Randomized_Designs <- function (..., Identifiers, Data_Frame = NULL, Number_of_Treatment_Groups, Number_of_Experimental_Units_in_Each_Treatment_Group, Variable_Weights = rep(1, ifelse(missing(Data_Frame), length(list(...)), ncol(Data_Frame[, c(which(colnames(Data_Frame) %in% sapply(match.call(expand.dots = FALSE)$..., deparse)))]))), Moment_Weighing_Method = c("Exponential (2 Base)", "Exponential (Golden Ratio Base)", "Ordinal", "Exponential (Euler's Number Base)", "None", "Custom Weights"), Custom_Moment_Weights = NULL, Number_of_Combinations_to_Report = ifelse(factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
}))) <= 3, factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
}))), ifelse((floor((factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
})))) * 0.01) <= 3) & (factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
}))) > 3), 3, ifelse(floor((factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
})))) * 0.01) > 3, floor((factorial(ifelse(missing(Data_Frame), unique(sapply(list(...), length)), nrow(Data_Frame))) / (factorial(Number_of_Treatment_Groups) * Reduce(`*`, sapply(seq_len(Number_of_Treatment_Groups), function (x) {
  factorial(Number_of_Experimental_Units_in_Each_Treatment_Group)
})))) * 0.01))))) {
  ...
  if ((!is.vector(Number_of_Combinations_to_Report)) | (length(Number_of_Combinations_to_Report) != 1)) {
    stop ("The 'Number_of_Combinations_to_Report' argument must be a vector of length 1.")
  }
  if (!is.finite(Number_of_Combinations_to_Report)) {
    stop ("The 'Number_of_Combinations_to_Report' argument must be finite.")
  }
  if (!is.numeric(Number_of_Combinations_to_Report)) {
    stop ("The 'Number_of_Combinations_to_Report' argument must be numeric")
  }
  if (Number_of_Combinations_to_Report < 1) {
    stop ("The 'Number_of_Combinations_to_Report' argument must be a positive number.")
  }
  if ((Number_of_Combinations_to_Report %% 1) != 0) {
    stop ("The 'Number_of_Combinations_to_Report' argument must be an integer.")
  }
  if (Moment_Weighing_Method[1] == "Exponential (2 Base)") {
    Moment_Weights <- rev(2 ^ (seq_len(Number_of_Experimental_Units_in_Each_Treatment_Group) - 1))
  } else if (Moment_Weighing_Method[1] == "Exponential (Golden Ratio Base)") {
  ...
  List_of_Possible_Groups <- lapply(All_Possible_Groups, function (x) {
    names(x) <- paste0("Group_", seq_len(Number_of_Treatment_Groups))
    x
  })
  ...
  Rescaled_Moments <- lapply(Rescaled_List_of_Possible_Groups, function (y) {
    lapply(y, function (z) {
      u <- sapply(z[, 2:ncol(z)], function (a) {
  ...
  Optimal_Combinations <- lapply(seq_len(Number_of_Combinations_to_Report), function (x) {
    list(Group_Assignments = Best_Possible_Grouping_Combinations[[x]], Moment_Information = Best_Possible_Grouping_Combinations_Moments[[x]])
  })
  names(Optimal_Combinations) <- paste0("The ", ifelse(substr(seq_len(Number_of_Combinations_to_Report), nchar(seq_len(Number_of_Combinations_to_Report)), nchar(seq_len(Number_of_Combinations_to_Report))) %in% c("0", "4", "5", "6", "7", "8", "9"), paste0(seq_len(Number_of_Combinations_to_Report), "th"), ifelse(substr(seq_len(Number_of_Combinations_to_Report), nchar(seq_len(Number_of_Combinations_to_Report)), nchar(seq_len(Number_of_Combinations_to_Report))) %in% c("1"), paste0(seq_len(Number_of_Combinations_to_Report), "st"), ifelse(substr(seq_len(Number_of_Combinations_to_Report), nchar(seq_len(Number_of_Combinations_to_Report)), nchar(seq_len(Number_of_Combinations_to_Report))) %in% c("2"), paste0(seq_len(Number_of_Combinations_to_Report), "nd"), ifelse(substr(seq_len(Number_of_Combinations_to_Report), nchar(seq_len(Number_of_Combinations_to_Report)), nchar(seq_len(Number_of_Combinations_to_Report))) %in% c("3"), paste0(seq_len(Number_of_Combinations_to_Report), "rd"), seq_len(Number_of_Combinations_to_Report))))), "-Best Combination")
  names(Optimal_Combinations)[1] <- gsub("1st-", "", names(Optimal_Combinations)[1])
