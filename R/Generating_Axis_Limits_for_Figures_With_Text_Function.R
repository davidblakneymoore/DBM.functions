...
#' @param Shift_Text_Vertically_Away_From_Their_Coordinates whether text should be shifted vertically away from the points they are asxociated with. For example, in many ordinations (such as those generated by principal component analysis or nonmetric multidimensional scaling), loading vectors are included in the plotting region, and these loading vectors are identified with their associated variable names. To prevent these variable names from being placed directly over the tips of the loading vectors, they may be shifted slightly upward (if the vector points in the upward direction) or downward (if the vector points in the downward direction). The default is `TRUE`.
#'
