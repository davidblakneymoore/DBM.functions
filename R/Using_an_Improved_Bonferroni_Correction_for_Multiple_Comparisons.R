#' Using an Improved Bonferroni Correction for Multiple Comparisons
#'
#' This function w
#'
#' `Using_an_Improved_Bonferroni_Correction_for_Multiple_Comparisons` w
#'
#' @param Comparisons w
#' @param p_Values w
#' @param Data_Frame w
...
#'
#' @return This function returns a data frame that contains the name of the comparisons or analyses, the original p values, the corrected p values (following this improved Bonferroni correction method), and a column indicating whether or not the corrected p values are statistically significant.
#'
...
#' 
#' # Using the Function
#' Using_an_Improved_Bonferroni_Correction_for_Multiple_Comparisons(Comparison, p_Value, Data_Frame_1)
#'
#' @export
Using_an_Improved_Bonferroni_Correction_for_Multiple_Comparisons <- function (Comparisons, p_Values, Data_Frame, Experimentwise_Type_I_Error_Rate = 0.05) {
  ...
  Data_Frame$Improved_Bonferroni_Correction_Adjusted_p_Value <- sapply(seq_len(nrow(Data_Frame)), function (x) {
    y <- Reduce(`+`, Data_Frame$p_Value[seq_len(x)])
    ifelse(y > 1, 1, y)
  })
  Data_Frame$Significance_Using_the_Improved_Sidak_Correction <- Data_Frame$Improved_Bonferroni_Correction_Adjusted_p_Value < Experimentwise_Type_I_Error_Rate
