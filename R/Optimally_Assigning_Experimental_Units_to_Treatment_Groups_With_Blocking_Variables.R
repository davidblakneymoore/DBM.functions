...
#'
#' @return This function returns optimal treatment group assignments (for cases when there are blocking variables). Since experimental units are assigned to treatment groups separately (in a completely independent fashion) by block, each of the main list elements of the function's output correspond to a different block.
#'
#' @author David B. Moore (\email{davidblakneymoore@@gmail.com})
#'
#' @examples
#' set.seed(2024)
#' Tree_Identifiers <- paste("Tree", seq_len(40), sep = "_")
#' Diameters_at_Breast_Height <- runif(40, 25, 50)
#' Heights <- runif(40, 30, 40)
#' Crown_Classes <- sample(c("Dominant", "Codominant", "Intermediate", "Overtopped"), 40, replace = TRUE)
#' Significant_Defect_Present <- sample(c("Yes", "No"), 40, replace = TRUE, prob = c(0.05, 0.95))
#' Blocks <- rep(1:4, each = 10, length.out = 40)
...
#' (The_Best_3_Grouping_Combinations_for_Each_Block <- lapply(Ordered_Grouping_Combinations_by_Block, `[`, seq_len(3)))
#'
#' @export
