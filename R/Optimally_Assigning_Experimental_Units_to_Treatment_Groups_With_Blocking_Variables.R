...
#' @param Blocking_Variable the names of the blocks. If there is more than one blocking variable, please use R's `interaction()` function to combine the different blocks into one variable before using the function.
...
#'
#' @return This function returns optimal treatment group assignments (for cases when there are blocking variables).
#'
...
#' set.seed(2024)
#' Number_of_Trees <- 40
#' Tree_Identifiers <- paste("Tree", seq_len(Number_of_Trees), sep = "_")
#' Diameters_at_Breast_Height <- runif(Number_of_Trees, 25, 50)
#' Heights <- runif(Number_of_Trees, 30, 40)
#' Crown_Classes <- sample(c("Dominant", "Codominant", "Intermediate", "Overtopped"), Number_of_Trees, replace = TRUE)
#' Significant_Defect_Present <- sample(c("Yes", "No"), Number_of_Trees, replace = TRUE, prob = c(0.05, 0.95))
#' Blocks <- rep(1:4, length.out = Number_of_Trees)
#' Data_Frame <- data.frame(Tree_Identifier = Tree_Identifiers, Diameter_at_Breast_Height = Diameters_at_Breast_Height, Height = Heights, Crown_Class = Crown_Classes, Significant_Defect_Present = Significant_Defect_Present, Block = Blocks)
...
#' (The_Best_3_Grouping_Combinations_for_Each_Block <- lapply(Ordered_Grouping_Combinations_by_Block, `[`, 1:3))
#'
#' @export
Optimally_Assigning_Experimental_Units_to_Treatment_Groups_With_Blocking_Variables <- function (..., Identifiers, Blocking_Variable, Data_Frame = NULL, Number_of_Treatment_Groups, Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group, Variable_Weights = rep(1, ifelse(missing(Data_Frame), length(list(...)), ncol(Data_Frame[, c(which(colnames(Data_Frame) %in% sapply(match.call(expand.dots = FALSE)$..., deparse)))]))), Moment_Weighing_Method = c("Exponential (2 Base)", "Exponential (Golden Ratio Base)", "Ordinal", "Exponential (Euler's Number Base)", "None", "Custom Weights"), Custom_Moment_Weights = NULL) {
  ...
  if ((Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group * Number_of_Treatment_Groups) > min(sapply(split(Data_Frame, Data_Frame$Block), nrow))) {
  ...
  Data_Frame <- data.frame(Identifiers, Blocks, Variables_for_the_Calculations)
  colnames(Data_Frame)[which(colnames(Data_Frame) == "Identifiers")] <- Identifiers_Name
  colnames(Data_Frame)[which(colnames(Data_Frame) == "Blocks")] <- Blocks_Name
  Data_Split_by_Block <- split(Data_Frame, Data_Frame[, which(colnames(Data_Frame) == Blocks_Name)])
  Rescaled_Data_by_Block <- lapply(Data_Split_by_Block, function (u) {
    Rescaled_Data <- as.data.frame(lapply(u[, 3:ncol(u)], function (x) {
      (x - mean(x)) / sd(x)
    }))
    Rescaled_Data[, Identifiers_Name] <- u[, Identifiers_Name]
    Rescaled_Data <- Rescaled_Data[, c(which(colnames(Rescaled_Data) == Identifiers_Name), which(colnames(Rescaled_Data) == Blocks_Name), which((colnames(Rescaled_Data) != Identifiers_Name) & (colnames(Rescaled_Data) != Blocks_Name)))]
    Rescaled_Data
  })
  ...
  Ordered_Grouping_Combinations_by_Block <- mapply(function (u, v) {
    ...
    Original_List_of_Possible_Groups <- lapply(List_of_Possible_Groups, function (w) {
      lapply(w, function (y) {
        ...
        z[, c(which(colnames(z) == Identifiers_Name), which(colnames(z) == Blocks_Name), which((colnames(z) != Identifiers_Name) & (colnames(z) != Blocks_Name)))]
      })
    })
    ...
    Ordered_Grouping_Combinations_Moments <- lapply(Ordered_Grouping_Combinations, function (x) {
      lapply(x, function (y) {
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 1) {
          Means <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], mean)
        }
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 2) {
          Variances <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], var)
        }
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 3) {
          Skewnesses <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], function (z) {
        ...
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 4) {
          Kurtoses <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], function (z) {
        ...
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 5) {
          Hyperskewnesses <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], function (z) {
            (1 / length(z)) * sum(((z - mean(z)) / sd(z)) ^ 5)
          })
        }
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 6) {
          Hypertailednesses <- sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], function (z) {
        ...
        if (Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group >= 7) {
          Higher_Order_Moments <- lapply(7:length(Number_of_Experimental_Units_From_Each_Block_in_Each_Treatment_Group), function (i) {
            sapply(y[, which((colnames(y) != Identifiers_Name) & (colnames(y) != Blocks_Name))], function (z) {
    ...
    Optimal_Combinations <- lapply(seq_len(length(Ordered_Grouping_Combinations_Moments)), function (x) {
      list(Group_Assignments = Best_Possible_Grouping_Combinations[[x]], Moment_Information = Ordered_Grouping_Combinations_Moments[[x]])
    })
