#' Optimally Assigning Experimental Units to Treatment Groups With No Blocking Variables
#'
...
#' Optimal_Group_Assignments <- Optimally_Assigning_Experimental_Units_to_Treatment_Groups_With_No_Blocking_Variables(Diameter_at_Breast_Height, Height, Crown_Class, Identifiers = Tree_Identifier, Data_Frame = Data_Frame, Number_of_Treatment_Groups = 3, Number_of_Experimental_Units_in_Each_Treatment_Group = 4)
#' Optimal_Group_Assignments[1:3]
#'
#' @export
Optimally_Assigning_Experimental_Units_to_Treatment_Groups_With_No_Blocking_Variables <- function (..., Identifiers, Data_Frame = NULL, Number_of_Treatment_Groups, Number_of_Experimental_Units_in_Each_Treatment_Group, Variable_Weights = rep(1, ifelse(missing(Data_Frame), length(list(...)), ncol(Data_Frame[, c(which(colnames(Data_Frame) %in% sapply(match.call(expand.dots = FALSE)$..., deparse)))]))), Moment_Weighing_Method = c("Exponential (2 Base)", "Exponential (Golden Ratio Base)", "Ordinal", "Exponential (Euler's Number Base)", "None", "Custom Weights"), Custom_Moment_Weights = NULL) {
