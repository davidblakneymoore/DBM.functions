...
#' @examples
#' set.seed(13)
#' Data_Frame <- data.frame(Tree_Identifier = paste("Tree", seq_len(13), sep = "_"), Diameter_at_Breast_Height = round(abs(rnorm(13, 30, 2.5)), 1), Height = round(abs(rnorm(13, 30, 2.5)), 1), Crown_Class = sample(c("Dominant", "Codominant", "Intermediate", "Overtopped"), 13, replace = TRUE), Significant_Defect_Present = sample(c("Yes", "No"), 13, replace = TRUE, prob = c(0.05, 0.95)))
