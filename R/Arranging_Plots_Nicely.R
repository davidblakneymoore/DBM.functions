...
#'
#' This function generates a plot layout matrix that is as square as possible - in other words, it generates a plot layout matrix whose number of rows and number of columns differ by either `0` or `1`.
#'
#' `Arranging_Plots_Nicely` generates a plot layout matrix that is as square as possible - in other words, it generates a plot layout matrix whose number of rows and number of columns differ by either `0` or `1`. This function works best when each individual plot in the figure is desired to be equal in size and in importance.
#'
...
#' (Layout_Matrix <- DBM.functions::Arranging_Plots(Number_of_Plots = (ncol(mtcars) - 3), Arrangement_Option = "Tall"))
#'
#' # Make the Figure
...
Arranging_Plots_Nicely <- function (Number_of_Plots, Arrangement_Option = c("Wide", "Tall"), Row_for_Plot_Title = TRUE, Row_for_Plot_Legend = TRUE) {
  ...
  if (is.null(Arrangement_Option) | is.na(Arrangement_Option)) {
