...
#'   plot(Data_Frame$mpg ~ Data_Frame[, which(!(colnames(Data_Frame) %in% c("mpg", "am")))[x]], xlab = colnames(Data_Frame)[which(!(colnames(Data_Frame) %in% c("mpg", "am")))[x]], ylab = "mpg", main = paste0("'mpg' and '", colnames(Data_Frame)[which(!(colnames(Data_Frame) %in% c("mpg", "am")))[x]], "'"), type = "n", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2)
...
#' legend("top", horiz = TRUE, title = expression(paste(bold("'am' Column Value"))), legend = unique(Data_Frame$am), col = 1:2, pch = 19, cex = 2)
#' dev.off()
#'
#' @export
