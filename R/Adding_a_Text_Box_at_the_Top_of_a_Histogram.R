...
#'
#' @export
Adding_a_Text_Box_at_the_Top_of_a_Histogram <- function (Values, Text, Text_Size = 1, Plot_Margins = c(5, 4, 4, 2) + 0.1, ...) {
  ...
  if ("breaks" %in% names(Extra_Arguments)) {
    if (is.numeric(Extra_Arguments$breaks)) {
      if (length(Extra_Arguments$breaks) == 1) {
        ...
        if ((min(Values) < min(Extra_Arguments$breaks)) | (max(Values) > max(Extra_Arguments$breaks))) {
    ...
    if (is.function(Extra_Arguments$breaks)) {
      ...
      if (!is.numeric(Breaks)) {
        ...
        if (length(Breaks) == 1) {
          ...
          if ((min(Values) < min(Breaks)) | (max(Values) > max(Breaks))) {
  ...
  if (!("freq" %in% names(Extra_Arguments))) {
    ...
    if (Extra_Arguments$freq == T) {
      Vertical_Axis_Coordinates <- Histogram_Information$density
    } else if (Extra_Arguments$freq == F) {
      Vertical_Axis_Coordinates <- Histogram_Information$counts
    }
  }
  Proportion_of_Space_Around_the_Histogram_Top_and_Bottom <- ((diff(Initial_Vertical_Axis_Limits) - diff(range(Vertical_Axis_Coordinates))) / diff(Initial_Vertical_Axis_Limits)) / 2
