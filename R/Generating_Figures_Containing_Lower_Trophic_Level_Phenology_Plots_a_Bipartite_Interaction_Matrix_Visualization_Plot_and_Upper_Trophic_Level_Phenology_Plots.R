...
#' @param Scale_Bar_Tick_Length a positive number specifying the relative lengths of the tick marks on the scale bar. The default value, `0.3`, w.
...
#' @param Scale_Bar_Factor a number between 0 and 1 (exclusive) that specifies approximately how tall the scale bar will be relative to the vertical axis range. The default value to this argument is `0.2`.
...
#' @param Relative_Plot_Heights the relative heights of the plots containing the titles and all the other plots. This argument must be a numeric vector contain two positive numbers. The default value to this argument is `c(1, 9)`.
#' @param Overall_Plot_Title_Font_Size the relative size of the titles of the two title plots. This argument must be a single positive number. The default value to this argument is `3.25`.
#' @param Subplot_Title_Font_Size the relative size of the titles of the lower-trophic-level violin-like phenology plots, the plot containing lower-trophic-level species names, the bipartite interaction visualization plot, the plot containing the higher-trophic-level species names, and the higher-trophic-level violin-like phenology plots. This argument must be a single positive number. The default value to this argument is `1.5`.
...
#' @param Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species whether species should be arranged in a way that minimizes the number of crossing edges (interactions) in the bipartite interaction visualization plot. If the number of crossing edges is minimized, the species will probably not be arranged alphabetically in the resulting figure. The default value, `FALSE`, causes the species from both trophic levels to be arranged alphabetically.
#'
...
#' # Example 1
#' set.seed(42)
#' List_of_Bipartite_Interaction_Matrices_by_Date_1 <- list("2024-04-05" = matrix(round(rchisq(24, 6)), ncol = 4), "2024-04-07" = matrix(round(rchisq(24, 6)), ncol = 4), "2024-04-09" = matrix(round(rchisq(24, 6)), ncol = 4))
...
#'   x
#' })
#' Generating_Figures_Containing_Lower_Trophic_Level_Phenology_Plots_a_Bipartite_Interaction_Matrix_Visualization_Plot_and_Upper_Trophic_Level_Phenology_Plots(List_of_Bipartite_Interaction_Matrices_by_Date_1, Scale_Bar_Factor = 0.25, Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species = TRUE)
#'
#' # Example 2
#' set.seed(47)
#' List_of_Bipartite_Interaction_Matrices_by_Date_2 <- list("2020-04-05" = matrix(round(rf(48, 5, 2)), ncol = 8), "2020-05-10" = matrix(round(rf(48, 5, 2)), ncol = 8), "2020-06-09" = matrix(round(rf(48, 5, 2)), ncol = 8), "2020-07-13" = matrix(round(rf(48, 5, 2)), ncol = 8))
...
#'   x
#' })
#' Generating_Figures_Containing_Lower_Trophic_Level_Phenology_Plots_a_Bipartite_Interaction_Matrix_Visualization_Plot_and_Upper_Trophic_Level_Phenology_Plots(List_of_Bipartite_Interaction_Matrices_by_Date_2)
#'
#' @export
Generating_Figures_Containing_Lower_Trophic_Level_Phenology_Plots_a_Bipartite_Interaction_Matrix_Visualization_Plot_and_Upper_Trophic_Level_Phenology_Plots <- function (List_of_Bipartite_Interaction_Matrices_by_Date, Gap_Size_Constant = 0.2, Relative_Bar_Width = 0.2, Lower_Trophic_Level_Color = "darkgreen", Higher_Trophic_Level_Color = "darkred", Interaction_Color = "black", Scale_Bar_Tick_Length = 0.3, Horizontal_Scale_Text_Shift_Constant = 0.1, Vertical_Scale_Title_Shift_Constant = 0.1, Scale_Bar_Factor = 0.2, Relative_Plot_Widths = c(11, 4, 10, 4, 11), Relative_Plot_Heights = c(1, 9), Overall_Plot_Title_Font_Size = 3.25, Subplot_Title_Font_Size = 1.5, Species_Name_Font_Size = 1.5, Scale_Bar_Title_Font_Size = 1, Scale_Bar_Text_Font_Size = 1, Horizontal_Axis_Title_Font_Size = 1, Horizontal_Axis_Dates_Font_Size = 1, Left_and_Right_Figure_Margins = 9, Between_Title_Plots_and_Other_Plots_Margin = 1, Bottom_Margin = 8, Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species = FALSE) {
  ...
  if (any(sapply(List_of_Bipartite_Interaction_Matrices_by_Date, function (x) {
    any(is.na(x))
  }))) {
    stop ("The matrices in the 'List_of_Bipartite_Interaction_Matrices_by_Date' argument must not contain any missing values.")
  }
  ...
  if (length(Scale_Bar_Tick_Length) != 1) {
    stop ("The 'Scale_Bar_Tick_Length' argument must be of length 1.")
  }
  if (is.na(Scale_Bar_Tick_Length)) {
    stop ("The 'Scale_Bar_Tick_Length' argument must must be provided.")
  }
  if (!is.numeric(Scale_Bar_Tick_Length)) {
    stop ("The 'Scale_Bar_Tick_Length' argument must be a number.")
  }
  if (Scale_Bar_Tick_Length <+ 0) {
    stop ("The 'Scale_Bar_Tick_Length' argument must be positive.")
  }
  ...
  if (length(Relative_Plot_Heights) != 2) {
    stop ("The 'Relative_Plot_Heights' argument must be of length 5.")
  }
  ...
  if (length(Subplot_Title_Font_Size) != 1) {
    stop ("The 'Subplot_Title_Font_Size' argument must be of length 1.")
  }
  if (is.na(Subplot_Title_Font_Size)) {
    stop ("The 'Subplot_Title_Font_Size' argument must must be provided.")
  }
  if (!is.numeric(Subplot_Title_Font_Size)) {
    stop ("The 'Subplot_Title_Font_Size' argument must be a number.")
  }
  if (Subplot_Title_Font_Size <= 0) {
    stop ("The 'Subplot_Title_Font_Size' argument must be positive.")
  }
  ...
  if (length(Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species) != 1) {
    stop ("The 'Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species' argument must be of length 1.")
  }
  if (is.na(Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species)) {
    stop ("The 'Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species' argument must not be missing.")
  }
  if (!is.logical(Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species)) {
    stop ("The 'Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species' must be of class 'logical'.")
  }
  ...
  if (Prioritize_Fewer_Crossing_Edges_Over_Alphabetized_Species == TRUE) {
    Optimally_Reordered_Matrix <- DBM.functions::Attempting_to_Diagonalize_a_Matrix_Based_on_Its_Full_Diagonal(Aggregated_Matrix)
    Aggregated_Matrix <- Optimally_Reordered_Matrix$Optimally_Reordered_Matrix
  ...
  Data_in_Long_Format_Split_by_Lower_Trophic_Level_Species <- Data_in_Long_Format_Split_by_Lower_Trophic_Level_Species[order(names(Data_in_Long_Format_Split_by_Lower_Trophic_Level_Species))]
  ...
  Cumulative_Lower_Trophic_Level_Heights_Split_by_Lower_Trophic_Level_Species <- Cumulative_Lower_Trophic_Level_Heights_Split_by_Lower_Trophic_Level_Species[order(names(Cumulative_Lower_Trophic_Level_Heights_Split_by_Lower_Trophic_Level_Species))]
  ...
  Data_in_Long_Format_Split_by_Higher_Trophic_Level_Species <- Data_in_Long_Format_Split_by_Higher_Trophic_Level_Species[order(names(Data_in_Long_Format_Split_by_Higher_Trophic_Level_Species))]
  ...
  Cumulative_Higher_Trophic_Level_Heights_Split_by_Higher_Trophic_Level_Species <- Cumulative_Higher_Trophic_Level_Heights_Split_by_Higher_Trophic_Level_Species[order(names(Cumulative_Higher_Trophic_Level_Heights_Split_by_Higher_Trophic_Level_Species))]
  ...
  Smaller_Scale_Bar_Numbers <- (10 ^ floor(log10(diff(Vertical_Axis_Limits) * Scale_Bar_Factor))) * c(0, 0.5, 1, 2)
  Larger_Scale_Bar_Numbers <- (10 ^ ceiling(log10(diff(Vertical_Axis_Limits) * Scale_Bar_Factor))) * c(0, 0.5, 1, 2)
  if (all(Smaller_Scale_Bar_Numbers == Larger_Scale_Bar_Numbers)) {
    Scale_Bar_Numbers <- (10 ^ round(log10(diff(Vertical_Axis_Limits) * Scale_Bar_Factor))) * c(0, 0.5, 1, 2)
  } else if (!all(Smaller_Scale_Bar_Numbers == Larger_Scale_Bar_Numbers)) {
    if (diff(c(min(Larger_Scale_Bar_Numbers), max(Larger_Scale_Bar_Numbers))) >= diff(Vertical_Axis_Limits)) {
      Scale_Bar_Numbers <- Smaller_Scale_Bar_Numbers
    } else if (diff(c(min(Larger_Scale_Bar_Numbers), max(Larger_Scale_Bar_Numbers))) < diff(Vertical_Axis_Limits)) {
      if (abs(diff(c(min(Larger_Scale_Bar_Numbers), max(Larger_Scale_Bar_Numbers))) - (diff(Vertical_Axis_Limits) * Scale_Bar_Factor)) > abs((diff(Vertical_Axis_Limits) * Scale_Bar_Factor) - diff(c(min(Smaller_Scale_Bar_Numbers), max(Smaller_Scale_Bar_Numbers))))) {
        Scale_Bar_Numbers <- Smaller_Scale_Bar_Numbers
      } else if (abs(diff(c(min(Larger_Scale_Bar_Numbers), max(Larger_Scale_Bar_Numbers))) - (diff(Vertical_Axis_Limits) * Scale_Bar_Factor)) < abs((diff(Vertical_Axis_Limits) * Scale_Bar_Factor) - diff(c(min(Smaller_Scale_Bar_Numbers), max(Smaller_Scale_Bar_Numbers))))) {
  ...
  layout(matrix(c(1, 1, 1, 2, 2, 2, 3, 4, 5, 5, 6, 7), nrow = 2, byrow = T), widths = Widths, heights = Heights)
  par(mar = c(1, 1, 1, 1))
  plot(0, xlab = "", ylab = "", axes = F, type = "n", main = "\nLower Trophic Level", cex.main = Overall_Plot_Title_Font_Size)
  plot(0, xlab = "", ylab = "", axes = F, type = "n", main = "\nHigher Trophic Level", cex.main = Overall_Plot_Title_Font_Size)
  par(mar = c(Bottom_Margin, Left_and_Right_Figure_Margins, Between_Title_Plots_and_Other_Plots_Margin, 1))
  plot(0, xlab = "", ylab = "", xlim = Date_Range, ylim = rev(Vertical_Axis_Limits), type = "n", axes = F, main = "Phenology", cex.main = Subplot_Title_Font_Size)
  ...
  plot(0, xlab = "", ylab = "", ylim = rev(Vertical_Axis_Limits), type = "n", axes = F, xpd = T, main = "Species", cex.main = Subplot_Title_Font_Size)
  sapply(seq_along(unique(rownames(Aggregated_Matrix))), function (i) {
  ...
  par(mar = c(Bottom_Margin, 1, Between_Title_Plots_and_Other_Plots_Margin, 1))
  plot(0, xlim = c(0, 10), ylim = rev(Vertical_Axis_Limits), xlab = "", ylab = "", type = "n", axes = F, main = "Bipartite Interaction Matrix Visualization", cex.main = Subplot_Title_Font_Size)
