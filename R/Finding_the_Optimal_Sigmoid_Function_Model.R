...
#'
#' This function determines which sigmoid model optimally fits a binary response data set (a data set where the response variable contains only `1`s and `0`s). Ten different, fully differentiable sigmoid functions are compared. The functions are the logistic function (`Response == 1 / (1 + exp(-(Intercept + (Slope * Predictor))))`), the hyperbolic tangent (`Response == (0.5 * tanh(Intercept + (Slope * Predictor))) + 0.5`), the arctangent function (`Response == (0.5 * ((2 / pi) * atan((pi / 2) * (Intercept + (Slope * Predictor))))) + 0.5`), the Gudermannian function (`Response == (2 / pi) * atan(tanh((Intercept + (Slope * Predictor)) * pi / 4)) + 0.5`), the error function (`Response == (0.5 * ((2 * pnorm((Intercept + (Slope * Predictor)) * sqrt(2), 0, 1)) - 1)) + 0.5`), a generalised logistic function (`Response == (1 + exp(-(Intercept + (Slope * Predictor)))) ^ (-Exponent)`), an algebraic function (`Response == (0.5 * ((Intercept + (Slope * Predictor)) / sqrt(1 + ((Intercept + (Slope * Predictor)) ^ 2)))) + 0.5`), a more generalised algebraic function (`Response == (0.5 * ((Intercept + (Slope * Predictor)) / ((1 + (abs(Intercept + (Slope * Predictor)) ^ Exponent)) ^ (1 / Exponent)))) + 0.5`), the Gompertz function (`Response == exp(-exp(Intercept + (Slope * Predictor)))`), and the Gompertz function that has been rotated by pi radians (`Response == 1 - (exp(-exp(Intercept + (Slope * Predictor))))`) (see \url{https://en.wikipedia.org/wiki/Sigmoid_function} for a description of some of these functions). Please note that all 10 of these functions are rotationally symmetric with the exception of the Gompertz function and the Gompertz function that has been rotated by pi radians - these two functions approach one asymptote much more quickly than they approach the other.
#'
#' `Finding_the_Optimal_Sigmoid_Function_Model` determines which sigmoid model optimally fits a binary response data set (a data set where the response variable contains only `1`s and `0`s).
#' 
...
#' 
#' # Run the Function
#' (Function_Output <- Finding_the_Optimal_Sigmoid_Function_Model(Predictor_Variable, Response_Variable, Data_Frame))
#' 
#' # Make a Plot
...
#' mtext("Predictor Variable", 1, line = 2.5)
#' mtext("Response Variable", 2, line = 2.5)
...
#' })
#' legend("bottom", xpd = TRUE, inset = c(0, -0.75), title = expression(paste(bold("Function (Pseudo r Squared Value)"))), legend = paste0(gsub("_", " ", gsub("_Model$", "", names(Function_Output$Model_Parameters))), " (", round(as.numeric(Function_Output$Output_Table[complete.cases(Function_Output$Output_Table), ]$Pseudo_r_Squared), 3), ")"), col = (seq_len(length(Function_Output$Model_Parameters)) + 1), lty = 1, lwd = 2.5)
#'
#' @export
Finding_the_Optimal_Sigmoid_Function_Model <- function (Predictor, Response, Data_Frame, Maximum_Number_of_Iterations = 10000) {
  ...
  Models_That_Are_Possible <- lapply(Models_That_Are_Possible, function (x) {
    x$Pseudo_r_Squared <- 1 - (sum(x$Model$m$resid() ^ 2) / Total_Sum_of_Squares)
  ...
  Output_Table <- as.data.frame(t(as.matrix(as.data.frame(lapply(Ordered_Models_That_Are_Possible, function (x) {
    c(Name = x$Name, Equation = x$Equation, Deviance = x$Deviance, Pseudo_r_Squared = x$Pseudo_r_Squared)
  })))))
