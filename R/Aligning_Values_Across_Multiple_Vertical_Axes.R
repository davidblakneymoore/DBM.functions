...
#' @param Axis_Buffers the minimum fraction of blank space you wish to leave around the top and bottom of the graph for each variable (above and below each variable's plotted points). The default is `0.05` - in other words, at least 5 % of the space on the top and the bottom of the graph will be empty above and below each variable's plotted points. This argument must be a finite number between `0` and `1` (inclusive).
#'
...
#' (Final_Vertical_Axis_Limits <- DBM.functions::Aligning_Values_Across_Multiple_Vertical_Axes(Variable_1, Variable_2, Variable_3, Data_Frame = Made_Up_Data_for_the_Aligning_Values_Across_Multiple_Vertical_Axes_Function, Values_to_Align = Values_to_Align, Variable_Weights = Variable_Weights))
#'
#' # Make some plots.
...
Aligning_Values_Across_Multiple_Vertical_Axes <- function (..., Data_Frame, Values_to_Align = rep(0, ifelse(missing(Data_Frame), length(list(...)), ncol(Data_Frame[, c(which(colnames(Data_Frame) %in% sapply(match.call(expand.dots = FALSE)$..., deparse)))]))), Variable_Weights = rep(1, ifelse(missing(Data_Frame), length(list(...)), ncol(Data_Frame[, c(which(colnames(Data_Frame) %in% sapply(match.call(expand.dots = FALSE)$..., deparse)))]))), Axis_Buffer = 0.05) {
  ...
  if (!is.numeric(Axis_Buffer) | (Axis_Buffer < 0) | (Axis_Buffer > 1) | (!is.finite(Axis_Buffer)) | (length(Axis_Buffer) != 1)) {
    stop ("The 'Axis_Buffer' argument must contain a single, finite, numeric value that is between 0 and 1 (inclusive).")
  }
