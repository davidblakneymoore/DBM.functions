...
#'
#' This function determines how to align values optimally across multiple vertical axes such that the specified values will align perfectly across all vertical axes.
#'
#' `Aligning_Values_Across_Multiple_Vertical_Axes` generates new axis limits for multiple vertical axes such that specified values on the various vertical axes are aligned vertically. The user defines the values to align. Though the example uses `base` R plotting functions, these new axis limits can be used with plotting functions from any package.
