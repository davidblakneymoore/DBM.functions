...
#'
#' For large matrices, this function may take an extremely long time to run because it performs all possible row and column exchanges and chooses the one that most closely matches the `Template_Matrix`.
#'
#' This function relies on another function from the `DBM.functions` package - the \link[DBM.functions]{Finding_the_Full_Diagonal_of_a_Matrix} function - as well as the \link[gtools]{permutations} function from the `gtools` package.
#'
...
#'
#' @examples
#' Matrix_1 <- matrix(rchisq(24, 5), ncol = 4)
#' Attempting_to_Diagonalize_a_Matrix_Based_on_Its_Full_Diagonal(Matrix_1)
#'
#' @export
Attempting_to_Diagonalize_a_Matrix_Based_on_Its_Full_Diagonal <- function (Matrix) {
  ...
  Row_Permutations <- DBM.functions::Generating_Permutations(nrow(Matrix))
  Row_Permutations <- do.call("rbind", lapply(Row_Permutations, function (x) {
    rownames(Matrix)[x]
  }))
  ...
  Column_Permutations <- DBM.functions::Generating_Permutations(ncol(Matrix))
  Column_Permutations <- do.call("rbind", lapply(Column_Permutations, function (x) {
    colnames(Matrix)[x]
  }))
