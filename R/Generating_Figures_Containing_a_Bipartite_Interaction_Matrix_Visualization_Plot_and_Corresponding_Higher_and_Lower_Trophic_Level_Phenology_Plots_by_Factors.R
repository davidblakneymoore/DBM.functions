#' Generating Figures Containing a Bipartite Interaction Matrix Visualization Plot and Corresponding Higher and Lower Trophic Level Phenology Plots by Factors
#'
...
#'
#' This function relies on the \link[DBM.functions]{Attempting_to_Diagonalize_a_Matrix_Based_on_Its_Full_Diagonal} function, the \link[DBM.functions]{Finding_the_Least_Common_Denominator_of_Imperfect_Fractions} function, the \link[DBM.functions]{Generating_Overall_Plot_Layout_Matrices} function, the \link[DBM.functions]{Making_Reasonable_Scale_Bars} function, and the \link[DBM.functions]{Making_Replicable_Layout_Matrices_for_Complicated_Figures} function, which are all also found in the `DBM.functions` package.
#'
#' @param Data_Frame a data frame containing information about the number of interactions between the higher- and lower-trophic-level species, the higher-trophic-level species, the lower-trophic-level species, the dates, and the factors that will be used to split the data to generate multiple panels for the resulting figure. This data frame must contain a column of dates (in `"Date"` format), a column of lower-trophic-level species (in `"character"` format), one or more columns of higher-trophic-level species (containing the names of these species as the column names and `"numeric"` entries that represent the numbers of interactions between each particular higher-trophic-level species and the corresponding lower-trophic-level species), and columns for each of the `Factors_to_Split_by` (each of these columns will have `"character"` entries that will correspond to factors of an experiment; multifactorial experimental designs must be perfectly balanced, meaning that the levels of each factor must be perfectly crossed with all of the levels of all of the other factors). This argument must be provided for the function to work.
...
#' @param Factors_to_Split_by a `"character"` vector containing the names of the columns in the `Data_Frame` data frame that correspond to the factors in the factorial experimental design. These entries of these columns represent the levels of each factor. This argument must be provided for the function to work.
#' @param Factor_Data_for_Plotting a data frame (of class `"data.frame"`) that contains two columns - one column, which must be named `"Factor"`, must have entries that identically match the names of the `Factors_to_Split_by` argument; the other column, which must be named `"Factor_Name_for_Plotting"`, must contain the names of the factors as the user of the function would like them to appear in the figure titles in the resulting figure. This argument is optional, and if it is omitted, the default is to use the elements of the `Factors_to_Split_by` argument in the figure titles in the final figure.
#' @param Factor_Level_Data_for_Plotting a data frame (of class `"data.frame"`) that contains three columns - one column, which must be named `"Factor"`, must have entries that identically match the names of the `Factors_to_Split_by` argument; one column, which must be named `"Level"`, must contain the levels of each factor exactly as they appear in the `Data_Frame` data frame in each factor's column; one column, which must be named `"Level_Name_for_Plotting"`, must contain the names of the factor levels as the user of the function would like them to appear in the figure titles in the resulting figure. This argument is optional, and if it is omitted, the default is to use the names of the factor levels as they appear in the `Data_Frame` data frame in the figure titles in the final figure.
#' @param Overall_Plot_Title_Factor_Arrangement for multifactorial experimental designs, how figure titles should be arranged. With multiple factors, and with each panel within the overall figure corresponding to one combination of factor levels, there will be multiple factors and factor levels appearing in the title of each panel. Should each factor and its corresponding factor level appear in the same line, separated by a semicolon, or should they appear in separate lines? The two possible values for this argument are `"On Different Lines"` and `"On the Same Lines"`, with the default value being `"On Different Lines"`.
...
#'
#' @export
Generating_Figures_Containing_a_Bipartite_Interaction_Matrix_Visualization_Plot_and_Corresponding_Higher_and_Lower_Trophic_Level_Phenology_Plots_by_Factors <- function (Data_Frame, Higher_Trophic_Level_Species, Lower_Trophic_Level_Species, Date, Factors_to_Split_by, Factor_Data_for_Plotting = NULL, Factor_Level_Data_for_Plotting = NULL, Overall_Plot_Title_Factor_Arrangement = "On Different Lines", Lower_Trophic_Level_Plot_Title = "Lower Trophic Level", Higher_Trophic_Level_Plot_Title = "Higher Trophic Level", Prioritize_Interaction_Strength_Over_Alphabetization_in_Species_Layout = FALSE, Lower_Trophic_Level_Color = "darkgreen", Higher_Trophic_Level_Color = "darkred", Interaction_Color = "black", Method_for_Aggregating = "Sum", Include_Scale_Bars = TRUE, Scale_Bar_Factor = 0.25, Approximate_Proportion_of_Lower_Trophic_Level_Species_to_Pool = 0, Approximate_Proportion_of_Higher_Trophic_Level_Species_to_Pool = 0, Lower_Trophic_Level_Species_Names_Data_Frame = NULL, Higher_Trophic_Level_Species_Names_Data_Frame = NULL, Relative_Plot_Widths = c(11, 4, 10, 4, 11), Relative_Plot_Heights = c(1, 1, 8), Already_Pooled_Lower_Trophic_Level_Species_Name = NULL, Already_Pooled_Higher_Trophic_Level_Species_Name = NULL, Gap_Size_Constant = 0.2, Relative_Bar_Width = 0.2, Horizontal_Scale_Text_Shift_Constant = 0.1, Vertical_Scale_Title_Shift_Constant = 0.1, Scale_Bar_Title_Font_Size = 1, Scale_Bar_Text_Font_Size = 1, Overall_Plot_Title_Font_Size = 3.25, Trophic_Level_Plot_Title_Font_Size = 2.5, Phenology_and_Bipartite_Plot_Title_Font_Size = 1.5, Species_Name_Font_Size = 1.5, Horizontal_Axis_Title_Font_Size = 1, Horizontal_Axis_Dates_Font_Size = 1, Left_and_Right_Figure_Margins = 9, Between_Title_Plots_and_Other_Plots_Margin = 1, Bottom_Margin = 8) {
  ...
  if (!is.character(Factors_to_Split_by)) {
    stop ("The 'Factors_to_Split_by' argument must be a vector of character strings.")
  }
  ...
  if (!all(Factors_to_Split_by %in% colnames(Data_Frame))) {
    stop ("Each element of the 'Factors_to_Split_by' argument must be a column name in the 'Data_Frame' data frame.")
  }
  if (!is.null(Factor_Data_for_Plotting)) {
    if (class(Factor_Data_for_Plotting) != "data.frame") {
      stop ("The 'Factor_Data_for_Plotting' argument must be of class 'data.frame'.")
    }
    if (!("Factor" %in% colnames(Factor_Data_for_Plotting))) {
      stop ("'Factor' must be the name of a column in the 'Factor_Data_for_Plotting' data frame.")
    }
    if (!("Factor_Name_for_Plotting" %in% colnames(Factor_Data_for_Plotting))) {
      stop ("'Factor_Name_for_Plotting' must be the name of a column in the 'Factor_Data_for_Plotting' data frame.")
    }
    if (!is.character(Factor_Data_for_Plotting$Factor)) {
      stop ("The 'Factor' column in the 'Factor_Data_for_Plotting' data frame must be of clas 'character'.")
    }
    if (!is.character(Factor_Data_for_Plotting$Factor_Name_for_Plotting)) {
      stop ("The 'Factor_Name_for_Plotting' column in the 'Factor_Data_for_Plotting' data frame must be of clas 'character'.")
    }
    if (!all(Factors_to_Split_by %in% Factor_Data_for_Plotting$Factor)) {
      stop ("Every element from the 'Factors_to_Split_by' vector must be present in the 'Factor' column in the 'Factor_Data_for_Plotting' data frame.")
    }
  }
  if (!is.null(Factor_Level_Data_for_Plotting)) {
    if (class(Factor_Level_Data_for_Plotting) != "data.frame") {
      stop ("The 'Factor_Level_Data_for_Plotting' argument must be of class 'data.frame'.")
    }
    if (!("Factor" %in% colnames(Factor_Level_Data_for_Plotting))) {
      stop ("'Factor' must be the name of a column in the 'Factor_Level_Data_for_Plotting' data frame.")
    }
    if (!("Level" %in% colnames(Factor_Level_Data_for_Plotting))) {
      stop ("'Level' must be the name of a column in the 'Factor_Level_Data_for_Plotting' data frame.")
    }
    if (!("Level_Name_for_Plotting" %in% colnames(Factor_Level_Data_for_Plotting))) {
      stop ("'Level_Name_for_Plotting' must be the name of a column in the 'Factor_Level_Data_for_Plotting' data frame.")
    }
    if (!is.character(Factor_Level_Data_for_Plotting$Factor)) {
      stop ("The 'Factor' column in the 'Factor_Level_Data_for_Plotting' data frame must be of clas 'character'.")
    }
    if (!is.character(Factor_Level_Data_for_Plotting$Level)) {
      stop ("The 'Level' column in the 'Factor_Level_Data_for_Plotting' data frame must be of clas 'character'.")
    }
    if (!is.character(Factor_Level_Data_for_Plotting$Level_Name_for_Plotting)) {
      stop ("The 'Level_Name_for_Plotting' column in the 'Factor_Level_Data_for_Plotting' data frame must be of clas 'character'.")
    }
    if (!identical(lapply(split(Factor_Level_Data_for_Plotting, Factor_Level_Data_for_Plotting$Factor), function (x) {
      unique(x$Level)
    }), setNames(lapply(Factors_to_Split_by, function (x) {
      unique(Data_Frame[, which(colnames(Data_Frame) == x)])
    }), Factors_to_Split_by))) {
      stop ("Every factor from the 'Factors_to_Split_by' vector must be present in the 'Factor' column in the 'Factor_Level_Data_for_Plotting' data frame, and every level of each factor that's found in the 'Data_Frame' data frame must also be included in the 'Factor_Level_Data_for_Plotting' data frame paired with (in the same row as) the factor that it belongs to.")
    }
  }
  if (length(Overall_Plot_Title_Factor_Arrangement) != 1) {
    stop ("The 'Overall_Plot_Title_Factor_Arrangement' argument must be of length 1.")
  }
  if (is.na(Overall_Plot_Title_Factor_Arrangement)) {
    stop ("The 'Overall_Plot_Title_Factor_Arrangement' argument must not be missing.")
  }
  if (!is.character(Overall_Plot_Title_Factor_Arrangement)) {
    stop ("The 'Overall_Plot_Title_Factor_Arrangement' argument must be a character string.")
  }
  if (!(Overall_Plot_Title_Factor_Arrangement %in% c("On Different Lines", "On the Same Line"))) {
    stop ("The 'Overall_Plot_Title_Factor_Arrangement' argument must be either 'On Different Lines' or 'On the Same Line'.")
  }
  ...
  if (!is.null(Higher_Trophic_Level_Species_Names_Data_Frame)) {
    ...
    if (!("Higher_Trophic_Level_Species_Name" %in% colnames(Higher_Trophic_Level_Species_Names_Data_Frame))) {
      stop ("The 'Higher_Trophic_Level_Species_Names_Data_Frame' data frame must contain a column named 'Higher_Trophic_Level_Species_Name' whose entries must identically match the names of the columns of the matrices (which represent the higher-trophic-level species).")
    }
    ...
    if (!all(Higher_Trophic_Level_Species_Names_Data_Frame$Higher_Trophic_Level_Species_Name %in% colnames(Data_Frame))) {
      stop ("Elements in the 'Higher_Trophic_Level_Species_Name' column from the 'Higher_Trophic_Level_Species_Names_Data_Frame' data frame must also be column names of the 'Data_Frame' data frame.")
    }
  }
  ...
  if (length(Relative_Plot_Heights) != 3) {
    stop ("The 'Relative_Plot_Heights' argument must be of length 3.")
  }
  ...
  Data_Frame <- Data_Frame[, which(colnames(Data_Frame) %in% c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species, Higher_Trophic_Level_Species))]
  # Check for missing values
  if (any(is.na(Data_Frame))) {
    stop ("There must not be any missing values in the data frame.")
  }
  List_of_Factors_for_Splitting_the_Data <- as.list(Data_Frame[, colnames(Data_Frame) %in% c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species)])
  List_of_Factors_for_Splitting_the_Data <- List_of_Factors_for_Splitting_the_Data[match(c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species), names(List_of_Factors_for_Splitting_the_Data))]
  Split_Data <- split(Data_Frame, List_of_Factors_for_Splitting_the_Data)
  Numbers_of_Rows <- sapply(Split_Data, nrow)
  All_Possibilities <- do.call("expand.grid", lapply(rev(c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species)), function (i) {
    unique(Data_Frame[, which(colnames(Data_Frame) == i)])
  }))
  All_Possibilities <- All_Possibilities[, rev(seq_len(ncol(All_Possibilities)))]
  colnames(All_Possibilities) <- c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species)
  Actual_Combinations <- as.data.frame(t(as.matrix(as.data.frame(strsplit(unique(as.character(unique(do.call("interaction", lapply(c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species), function (i) {
    Data_Frame[, which(colnames(Data_Frame) == i)]
  }))))), "[.]")))))
  rownames(Actual_Combinations) <- NULL
  colnames(Actual_Combinations) <- c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species)
  if (nrow(All_Possibilities) == nrow(Actual_Combinations)) {
    Vector <- F
  } else if (nrow(All_Possibilities) != nrow(Actual_Combinations)) {
    Vector <- sapply(seq_len(nrow(All_Possibilities)), function (x) {
      all(!sapply(seq_len(nrow(Actual_Combinations)), function (y) {
        all(All_Possibilities[x, ] == Actual_Combinations[y, ])
      }))
    })
  }
  if ((!all(Numbers_of_Rows == max(Numbers_of_Rows))) | (any(Vector == T))) {
    Missing_Data_Frame <- as.data.frame(t(as.matrix(as.data.frame(strsplit(names(Numbers_of_Rows)[which(Numbers_of_Rows != max(Numbers_of_Rows))], "[.]")))))
    if (Reduce(`*`, dim(Missing_Data_Frame)) > 0) {
      rownames(Missing_Data_Frame) <- NULL
      colnames(Missing_Data_Frame) <- c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species)
    }
    Missing_Data_Number_of_Rows <- setNames(Numbers_of_Rows[which(Numbers_of_Rows != max(Numbers_of_Rows))], NULL)
    Missing_Data_Frame <- Missing_Data_Frame[order(Missing_Data_Number_of_Rows), ]
    Missing_Data_Number_of_Rows <- Missing_Data_Number_of_Rows[order(Missing_Data_Number_of_Rows)]
    First_Batch_of_Error_Messages <- lapply(seq_len(nrow(Missing_Data_Frame)), function (i) {
      Missing_Factors_and_Factor_Levels <- mapply(function (a, b) {
        paste0("the level of ", a, " is ", Missing_Data_Frame[i, b])
      }, a = c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species), b = seq_len(ncol(Missing_Data_Frame)), SIMPLIFY = F)
      Error_Message_Framework <- paste0(Reduce(function (a, b) {
        paste0(a, ", ", b)
      }, Missing_Factors_and_Factor_Levels[seq_len((length(Missing_Factors_and_Factor_Levels)) - 1)]), ", and ", Missing_Factors_and_Factor_Levels[length(Missing_Factors_and_Factor_Levels)])
      if (Missing_Data_Number_of_Rows[i] == 1) {
        Error_Message <- paste0("There is only ", Missing_Data_Number_of_Rows[i], " row where ", Error_Message_Framework)
      } else if (Missing_Data_Number_of_Rows[i] > 1) {
        Error_Message <- paste0("There are only ", Missing_Data_Number_of_Rows[i], " rows where ", Error_Message_Framework)
      }
      Error_Message
    })
    Missing_Combinations <- All_Combinations[which(Vector), ]
    Second_Batch_of_Error_Messages <- lapply(seq_len(nrow(Missing_Combinations)), function (i) {
      Missing_Factors_and_Factor_Levels <- mapply(function (a, b) {
        if (a %in% c(Date, Lower_Trophic_Level_Species)) {
          paste0("the ", a, " is ", Missing_Combinations[i, b])
        } else if (a %in% Factors_to_Split_by) {
          paste0("the level of ", a, " is ", Missing_Combinations[i, b])
        }
      }, a = c(Factors_to_Split_by, Date, Lower_Trophic_Level_Species), b = seq_len(ncol(Missing_Combinations)), SIMPLIFY = F)
      if (length(Factors_to_Split_by) == 0) {
        Error_Message_Framework <- Reduce(function (a, b) {
          paste0(a, " and ", b)
        }, Missing_Factors_and_Factor_Levels)
      } else if (length(Factors_to_Split_by) > 0) {
        Error_Message_Framework <- paste0(Reduce(function (a, b) {
          paste0(a, ", ", b)
        }, Missing_Factors_and_Factor_Levels[seq_len((length(Missing_Factors_and_Factor_Levels)) - 1)]), ", and ", Missing_Factors_and_Factor_Levels[length(Missing_Factors_and_Factor_Levels)])
      }
      paste0("There are no rows where ", Error_Message_Framework)
    })
    Error_Messages <- c(Second_Batch_of_Error_Messages, First_Batch_of_Error_Messages)
    stop (paste0("The data set is not balanced. ", Reduce(function (x, y) {
      paste0(x, ". ", y)
    }, Error_Messages), ". There should be ", max(Numbers_of_Rows), ifelse(max(Numbers_of_Rows) == 1, " row", " rows"), " for ", ifelse(length(Error_Messages) > 1, "each", "this"), " case."))
  }
  Data_Frame <- do.call("rbind", lapply(Split_Data, function (x) {
    Matrix_of_Higher_Trophic_Level_Species <- x[, which(colnames(x) %in% Higher_Trophic_Level_Species)]
    Metadata <- x[, which(!(colnames(x) %in% Higher_Trophic_Level_Species))]
    if (Method_for_Aggregating == "Sum") {
      Aggregated_Data <- as.data.frame(t(colSums(Matrix_of_Higher_Trophic_Level_Species)))
    } else if (Method_for_Aggregating == "Average") {
      Aggregated_Data <- as.data.frame(t(colMeans(Matrix_of_Higher_Trophic_Level_Species)))
    }
    Metadata <- as.data.frame(lapply(Metadata, unique))
    cbind(Metadata, Aggregated_Data)
  }))
  rownames(Data_Frame) <- NULL
  Splitting_Data_Frames_Into_Nested_Lists_by_Factors <- function (Data_Frame, Factors_to_Split_by) {
    if (length(Factors_to_Split_by) == 1) {
      split(Data_Frame, Data_Frame[Factors_to_Split_by])
    } else if (length(Factors_to_Split_by) > 1) {
      lapply(split(Data_Frame, Data_Frame[Factors_to_Split_by[1]]), Splitting_Data_Frames_Into_Nested_Lists_by_Factors, Factors_to_Split_by = Factors_to_Split_by[-1])
    }
  }
  Split_Data <- Splitting_Data_Frames_Into_Nested_Lists_by_Factors(Data_Frame, Factors_to_Split_by)
  Splitting_the_Data_by_Date_and_by_Lower_Trophic_Level_Species <- function (g) {
    if (is.data.frame(g)) {
      Metadata <- setNames(sapply(Factors_to_Split_by, function (y) {
        unique(g[, which(colnames(g) == y)])
      }), Factors_to_Split_by)
      List_of_Bipartite_Interaction_Matrices_by_Date <- lapply(split(g, g[, which(colnames(g) == Date)]), function (y) {
        Community_Matrix <- as.matrix(y[, which(colnames(y) %in% Higher_Trophic_Level_Species)])
        rownames(Community_Matrix) <- y[, which(colnames(y) == Lower_Trophic_Level_Species)]
        Community_Matrix
      })
      ...
      if (Prioritize_Interaction_Strength_Over_Alphabetization_in_Species_Layout == TRUE) {
        ...
        Optimally_Reordered_Matrix <- Attempting_to_Diagonalize_a_Matrix_Based_on_Its_Full_Diagonal(Aggregated_Matrix_to_Reorder)
      ...
      if (Include_Scale_Bars == TRUE) {
        Scale_Bar_Numbers <- Making_Reasonable_Scale_Bars(Vertical_Axis_Limits, Scale_Bar_Factor = Scale_Bar_Factor, Rounding_Method = "Up")
      ...
      Overall_Plot_Title <- Reduce(function (i, j) {
        if (Overall_Plot_Title_Factor_Arrangement == "On the Same Line") {
          paste0(i, "; ", j)
        } else if (Overall_Plot_Title_Factor_Arrangement == "On Different Lines") {
          paste0(i, "\n", j)
        }
      }, lapply(seq_along(Metadata), function (i) {
        if (is.null(Factor_Data_for_Plotting)) {
          j <- names(Metadata)[i]
        } else if (!is.null(Factor_Data_for_Plotting)) {
          j <- Factor_Data_for_Plotting$Factor_Name_for_Plotting[which(Factor_Data_for_Plotting$Factor == names(Metadata)[i])]
        }
        if (is.null(Factor_Level_Data_for_Plotting)) {
          k <- Metadata[i]
        } else if (!is.null(Factor_Level_Data_for_Plotting)) {
          k <- Factor_Level_Data_for_Plotting$Level_Name_for_Plotting[which((Factor_Level_Data_for_Plotting$Level == Metadata[i]) & (Factor_Level_Data_for_Plotting$Factor == names(Metadata)[i]))]
        }
        paste0(j, ": ", k)
      }))
      list(Aggregated_Matrix = Aggregated_Matrix, Number_of_Higher_Trophic_Levels = Number_of_Higher_Trophic_Levels, Number_of_Lower_Trophic_Levels = Number_of_Lower_Trophic_Levels, Cumulative_Higher_Trophic_Level_Heights_Data_Frame = Cumulative_Higher_Trophic_Level_Heights_Data_Frame, Cumulative_Lower_Trophic_Level_Heights_Data_Frame = Cumulative_Lower_Trophic_Level_Heights_Data_Frame, Data_in_Long_Format = Data_in_Long_Format, Higher_Trophic_Level_Species_Abundances_by_Date = Higher_Trophic_Level_Species_Abundances_by_Date, Lower_Trophic_Level_Species_Abundances_by_Date = Lower_Trophic_Level_Species_Abundances_by_Date, Overall_Plot_Title = Overall_Plot_Title, Vertical_Axis_Limits = Vertical_Axis_Limits, Dates = Dates, Date_Range = Date_Range, Date_Labels = Date_Labels, Scale_Bar_Numbers = Scale_Bar_Numbers, Vertical_Scale_Bar_Tick_Coordinates = Vertical_Scale_Bar_Tick_Coordinates)
    } else if (!is.data.frame(g)) {
      lapply(g, Splitting_the_Data_by_Date_and_by_Lower_Trophic_Level_Species)
    }
  }
  Plotting_Information <- lapply(Split_Data, Splitting_the_Data_by_Date_and_by_Lower_Trophic_Level_Species)
  Plotting_Information_Names <- c("Aggregated_Matrix", "Number_of_Higher_Trophic_Levels", "Number_of_Lower_Trophic_Levels", "Cumulative_Higher_Trophic_Level_Heights_Data_Frame", "Cumulative_Lower_Trophic_Level_Heights_Data_Frame", "Data_in_Long_Format", "Higher_Trophic_Level_Species_Abundances_by_Date", "Lower_Trophic_Level_Species_Abundances_by_Date", "Overall_Plot_Title", "Vertical_Axis_Limits", "Dates", "Date_Range", "Date_Labels", "Scale_Bar_Numbers", "Vertical_Scale_Bar_Tick_Coordinates")
  ...
  New_Widths <- Widths / sum(Widths)
  Heights <- Relative_Plot_Heights / sum(Relative_Plot_Heights)
  New_Widths <- Finding_the_Least_Common_Denominator_of_Imperfect_Fractions(New_Widths) * New_Widths
  New_Heights <- Finding_the_Least_Common_Denominator_of_Imperfect_Fractions(Heights) * Heights
  Individual_Layout_Matrix <- matrix(c(1, 1, 1, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 5, 6, 6, 7, 8), nrow = 3, byrow = T)
  Intermediate_Matrix <- do.call("rbind", lapply(seq_len(nrow(Individual_Layout_Matrix)), function (x) {
    unlist(sapply(seq_along(Individual_Layout_Matrix[x, ]), function (y) {
      rep(Individual_Layout_Matrix[x, y], each = New_Widths[y])
    }))
  }))
  Final_Individual_Layout_Matrix <- do.call("cbind", lapply(seq_len(ncol(Intermediate_Matrix)), function (x) {
  ...
  Final_Layout_Matrix <- Making_Replicable_Layout_Matrices_for_Complicated_Figures(Final_Individual_Layout_Matrix, Overall_Layout_Matrix)
  Making_Final_Figures <- function (x) {
    if (identical(names(x), Plotting_Information_Names)) {
      par(mar = c(1, 1, 5, 1))
      plot(0, xlab = "", ylab = "", axes = F, type = "n", main = paste0("\n\n", x$Overall_Plot_Title), cex.main = Overall_Plot_Title_Font_Size)
      par(mar = c(1, 1, 1, 1))
      ...
      plot(0, xlab = "", ylab = "", xlim = x$Date_Range, ylim = rev(x$Vertical_Axis_Limits), type = "n", axes = F, main = "Phenology", cex.main = Phenology_and_Bipartite_Plot_Title_Font_Size)
      axis.Date(1, at = x$Date_Labels, cex.axis = Horizontal_Axis_Dates_Font_Size)
      mtext("Date", 1, line = 4.5, cex = Horizontal_Axis_Title_Font_Size)
      sapply(seq_along(unique(rownames(x$Aggregated_Matrix))), function (i) {
        Lower_Trophic_Level_Species <- unique(rownames(x$Aggregated_Matrix))[i]
        Lower_Bounds <- (x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[which(x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame$Lower_Trophic_Level_Species == Lower_Trophic_Level_Species), ]$Midpoint) - (x$Lower_Trophic_Level_Species_Abundances_by_Date[which(rownames(x$Lower_Trophic_Level_Species_Abundances_by_Date) == Lower_Trophic_Level_Species), ] / 2)
        Upper_Bounds <- (x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[which(x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame$Lower_Trophic_Level_Species == Lower_Trophic_Level_Species), ]$Midpoint) + (x$Lower_Trophic_Level_Species_Abundances_by_Date[which(rownames(x$Lower_Trophic_Level_Species_Abundances_by_Date) == Lower_Trophic_Level_Species), ] / 2)
        polygon(c(x$Dates, rev(x$Dates), x$Dates[1]), c(Upper_Bounds, rev(Lower_Bounds), Upper_Bounds[1]), col = Lower_Trophic_Level_Color)
      })
      if (Include_Scale_Bars == TRUE) {
        ...
        segments(Horizontal_Scale_Bar_Coordinate, x$Vertical_Scale_Bar_Tick_Coordinates[1], Horizontal_Scale_Bar_Coordinate, x$Vertical_Scale_Bar_Tick_Coordinates[length(x$Vertical_Scale_Bar_Tick_Coordinates)], xpd = T)
        sapply(seq_len(length(x$Scale_Bar_Numbers)), function (i) {
          segments(Horizontal_Scale_Bar_Coordinate, x$Vertical_Scale_Bar_Tick_Coordinates[i], (Horizontal_Scale_Bar_Coordinate - (abs(diff(Horizontal_Axis_Coordinates)) * 0.1)), x$Vertical_Scale_Bar_Tick_Coordinates[i], xpd = T)
        })
        sapply(seq_len(length(x$Scale_Bar_Numbers)), function (i) {
          text((Horizontal_Scale_Bar_Coordinate - (abs(diff(Horizontal_Axis_Coordinates)) * (0.1 + Horizontal_Scale_Text_Shift_Constant))), x$Vertical_Scale_Bar_Tick_Coordinates[i], x$Scale_Bar_Numbers[i], xpd = T, cex = Scale_Bar_Text_Font_Size)
        })
        text((Horizontal_Scale_Bar_Coordinate - (abs(diff(Horizontal_Axis_Coordinates)) * (0.1 + Horizontal_Scale_Text_Shift_Constant))), (x$Vertical_Scale_Bar_Tick_Coordinates[length(x$Vertical_Scale_Bar_Tick_Coordinates)] - (abs(diff(x$Vertical_Axis_Limits)) * Vertical_Scale_Title_Shift_Constant)), "Total\nNumber of\nObserved\nInteractions", xpd = T, cex = Scale_Bar_Title_Font_Size)
      }
      par(mar = c(Bottom_Margin, 1, Between_Title_Plots_and_Other_Plots_Margin, 1))
      plot(0, xlab = "", ylab = "", ylim = rev(x$Vertical_Axis_Limits), type = "n", axes = F, xpd = T, main = "Species", cex.main = Phenology_and_Bipartite_Plot_Title_Font_Size)
      if (is.null(Lower_Trophic_Level_Species_Names_Data_Frame)) {
        sapply(seq_along(unique(rownames(x$Aggregated_Matrix))), function (i) {
          text(mean(c(par("usr")[1:2])), x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame$Midpoint, rownames(x$Aggregated_Matrix), xpd = T, cex = Species_Name_Font_Size)
        })
      } else if (!is.null(Lower_Trophic_Level_Species_Names_Data_Frame)) {
        sapply(seq_along(rownames(x$Aggregated_Matrix)), function (i) {
          text(mean(c(par("usr")[1:2])), x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame$Midpoint[i], ifelse(rownames(x$Aggregated_Matrix)[i] %in% Lower_Trophic_Level_Species_Names_Data_Frame$Lower_Trophic_Level_Species_Name, tryCatch (eval(parse(text = Lower_Trophic_Level_Species_Names_Data_Frame$Names_for_Plotting[match(rownames(x$Aggregated_Matrix), Lower_Trophic_Level_Species_Names_Data_Frame$Lower_Trophic_Level_Species_Name)][i])), error = function (e) {
            rownames(x$Aggregated_Matrix)[i]
          }), rownames(x$Aggregated_Matrix)[i]), xpd = T, cex = Species_Name_Font_Size)
        })
      }
      par(mar = c(Bottom_Margin, 1, Between_Title_Plots_and_Other_Plots_Margin, 1))
      plot(0, xlim = c(0, 10), ylim = rev(x$Vertical_Axis_Limits), xlab = "", ylab = "", type = "n", axes = F, main = "Bipartite Interaction Matrix Visualization", cex.main = Phenology_and_Bipartite_Plot_Title_Font_Size)
      sapply(seq_len(x$Number_of_Lower_Trophic_Levels), function (i) {
        polygon(c(0, (10 * Relative_Bar_Width), (10 * Relative_Bar_Width), 0, 0), c(x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound, x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound, x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[i, ]$Lower_Bound, x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[i, ]$Lower_Bound, x$Cumulative_Lower_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound), col = Lower_Trophic_Level_Color)
      })
      sapply(seq_len(x$Number_of_Higher_Trophic_Levels), function (i) {
        polygon(c((10 - (10 * Relative_Bar_Width)), 10, 10, (10 - (10 * Relative_Bar_Width)), (10 - (10 * Relative_Bar_Width))), c(x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound, x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound, x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[i, ]$Lower_Bound, x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[i, ]$Lower_Bound, x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[i, ]$Upper_Bound), col = Higher_Trophic_Level_Color)
      })
      sapply(seq_len(nrow(x$Data_in_Long_Format)), function (i) {
        polygon(c((10 * Relative_Bar_Width), (10 - (10 * Relative_Bar_Width)), (10 - (10 * Relative_Bar_Width)), (10 * Relative_Bar_Width), (10 * Relative_Bar_Width)), c(x$Data_in_Long_Format$Lower_Trophic_Level_Lower_Bound[i], x$Data_in_Long_Format$Higher_Trophic_Level_Lower_Bound[i], x$Data_in_Long_Format$Higher_Trophic_Level_Upper_Bound[i], x$Data_in_Long_Format$Lower_Trophic_Level_Upper_Bound[i], x$Data_in_Long_Format$Lower_Trophic_Level_Lower_Bound[i]), col = Interaction_Color)
      })
      par(mar = c(Bottom_Margin, 1, Between_Title_Plots_and_Other_Plots_Margin, 1))
      plot(0, xlab = "", ylab = "", ylim = rev(x$Vertical_Axis_Limits), type = "n", axes = F, xpd = T, main = "Species", cex.main = Phenology_and_Bipartite_Plot_Title_Font_Size)
      if (is.null(Higher_Trophic_Level_Species_Names_Data_Frame)) {
        sapply(seq_along(unique(colnames(x$Aggregated_Matrix))), function (i) {
          text(mean(c(par("usr")[1:2])), x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame$Midpoint, colnames(x$Aggregated_Matrix), xpd = T, cex = Species_Name_Font_Size)
        })
      } else if (!is.null(Higher_Trophic_Level_Species_Names_Data_Frame)) {
        sapply(seq_along(colnames(x$Aggregated_Matrix)), function (i) {
          text(mean(c(par("usr")[1:2])), x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame$Midpoint[i], ifelse(colnames(x$Aggregated_Matrix)[i] %in% Higher_Trophic_Level_Species_Names_Data_Frame$Higher_Trophic_Level_Species_Name, tryCatch (eval(parse(text = Higher_Trophic_Level_Species_Names_Data_Frame$Names_for_Plotting[match(colnames(x$Aggregated_Matrix), Higher_Trophic_Level_Species_Names_Data_Frame$Higher_Trophic_Level_Species_Name)][i])), error = function (e) {
            colnames(x$Aggregated_Matrix)[i]
          }), colnames(x$Aggregated_Matrix)[i]), xpd = T, cex = Species_Name_Font_Size)
        })
      }
      if (Include_Scale_Bars == TRUE) {
      ...
      mtext("Date", 1, line = 4.5, cex = Horizontal_Axis_Title_Font_Size)
      sapply(seq_along(unique(colnames(x$Aggregated_Matrix))), function (i) {
        Higher_Trophic_Level_Species <- unique(colnames(x$Aggregated_Matrix))[i]
        Lower_Bounds <- (x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[which(x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame$Higher_Trophic_Level_Species == Higher_Trophic_Level_Species), ]$Midpoint) - (x$Higher_Trophic_Level_Species_Abundances_by_Date[which(rownames(x$Higher_Trophic_Level_Species_Abundances_by_Date) == Higher_Trophic_Level_Species), ] / 2)
        Upper_Bounds <- (x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame[which(x$Cumulative_Higher_Trophic_Level_Heights_Data_Frame$Higher_Trophic_Level_Species == Higher_Trophic_Level_Species), ]$Midpoint) + (x$Higher_Trophic_Level_Species_Abundances_by_Date[which(rownames(x$Higher_Trophic_Level_Species_Abundances_by_Date) == Higher_Trophic_Level_Species), ] / 2)
        polygon(c(x$Dates, rev(x$Dates), x$Dates[1]), c(Upper_Bounds, rev(Lower_Bounds), Upper_Bounds[1]), col = Higher_Trophic_Level_Color)
      })
      if (Include_Scale_Bars == TRUE) {
        Horizontal_Axis_Coordinates <- par("usr")[1:2]
    ...
    } else if (!identical(names(x), Plotting_Information_Names)) {
      lapply(x, Making_Final_Figures)
    }
  }
  layout(Final_Layout_Matrix)
  Making_Final_Figures(Plotting_Information)
}
