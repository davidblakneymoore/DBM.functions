...
Adding_a_Text_Box_to_the_Top_or_Bottom_of_a_Plot <- function (Horizontal_Coordinates, Vertical_Coordinates, Text, Text_Box_Location = "Top", Text_Size = 1, Plot_Margins = c(5, 4, 4, 2) + 0.1, Tolerance = sqrt(.Machine$double.eps)) {
  ...
  if (!is.numeric(Plot_Margins)) {
    stop ("The 'Plot_Margins' argument must be of class 'numeric'.")
  }
  if (length(Plot_Margins) != 4) {
    stop ("The 'Plot_Margins' argument must be of length 4.")
  }
  if (any(is.na(Plot_Margins))) {
    stop ("The 'Plot_Margins' argument must not contain any missing values.")
  }
  if (any(Plot_Margins < 0)) {
    stop ("The 'Plot_Margins' argument must not contain only nonnegative numbers'.")
  }
  if (!is.numeric(Tolerance)) {
    stop ("The 'Tolerance' argument must be of class 'numeric'.")
  }
  if (length(Tolerance) != 1) {
    stop ("The 'Tolerance' argument must be of length 1.")
  }
  if (is.na(Tolerance)) {
    stop ("The 'Tolerance' argument must not contain any missing values.")
  }
  if (Tolerance <= 0) {
