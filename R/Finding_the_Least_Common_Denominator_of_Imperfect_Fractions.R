...
Finding_the_Least_Common_Denominator_of_Imperfect_Fractions <- function (Vector) {
  if (is.null(Vector)) {
    stop ("The 'Vector' argument must be present.")
  }
  if (is.list(Vector)) {
    stop ("The 'Vector' argument must not be a list.")
  }
  if (any(is.na(Vector))) {
    stop ("The 'Vector' argument must not contain any missing values.")
  }
  if (!is.numeric(Vector)) {
    stop ("The 'Vector' argument must be numeric.")
  }
  if (any(Vector <= 0)) {
    stop ("The 'Vector' argument must contain positive numbers only.")
  }
  Multiplier <- 1
  repeat {
