...
#'
#' This function calculates, for a set of observations, [standardized moments](https://en.wikipedia.org/wiki/Standardized_moment). It assumes that the observations are a population, not a sample, and therefore it does not perform any bias corrections. This function relies on the `Ordinalizing_Integers` function which is also found in the `DBM.functions` package.
#'
...
Calculating_Standardized_Moments <- function (Values, Moments) {
  ...
  if (max(Moments) <= 6) {
    ...
    Moment_Names <- c(Moment_Names, paste0(DBM.functions::Ordinalizing_Integers(setdiff(seq_len(max(Moments)), seq_along(Moment_Names))), "-Order Moment"))
  }
